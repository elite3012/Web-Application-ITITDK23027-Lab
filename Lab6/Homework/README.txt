...
- Password: password123
- Role: user
- Access: View-only (no Add/Edit/Delete buttons)
...
1. AuthFilter (checks login status for all routes)
2. AdminFilter (checks admin role for /student CRUD actions)
3. Controller (processes request)

AUTHENTICATION FLOW:
1. User accesses protected page -> AuthFilter checks session
2. If no session -> redirect to /login
3. User submits login form -> LoginController.doPost()
4. UserDAO.authenticate() verifies username/password with BCrypt
5. If valid -> create session, store User object, redirect to /student
6. If invalid -> show error message

AUTHORIZATION FLOW:
1. User (non-admin) tries CRUD action -> AdminFilter checks role
2. If not admin -> redirect with "Access denied" error
3. If admin -> proceed to controller
4. UI hides admin-only buttons for non-admin users
...
- Query timeout (10 seconds) to prevent hanging
- Resource cleanup in finally blocks
- Role-based UI hiding (not just disabling)
- Admin check in both filter AND controller (defense in depth)
- Proper session invalidation on logout
- Redirect after POST (PRG pattern)

TIME SPENT: Approximately 6-8 hours
- Database setup & User model: 1 hour
- Login/Logout & DAO: 1.5 hours
- Dashboard & Views: 1 hour
- AuthFilter: 0.5 hours
- AdminFilter: 0.5 hours
- Role-based UI updates: 1 hour
- Change Password feature: 1.5 hours
...
- Requires BCrypt library (jbcrypt-0.4.jar)
- Default session timeout: 30 minutes (configurable in web.xml)
- Tested on Tomcat 10.1.x with Jakarta EE 10

CONCLUSION:
All 8 exercises completed successfully with comprehensive security measures,
role-based access control, and enhanced UI/UX. The application properly implements
authentication using filters, authorization using role checks, and secure password
management using BCrypt hashing.
